...
import { ALL_QUESTIONS, DIM_META, YESNO_TEXT } from "../lib/questions";
    ...
    <div style={{ minHeight: "100vh", background: "#0F172A", display: "flex", flexDirection: "column", fontFamily: "'Noto Sans KR', sans-serif" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "32px 24px" }}>
        <div style={{ maxWidth: 640, width: "100%", opacity: fade ? 1 : 0, transition: "opacity 0.2s ease" }}>
          {/* 카테고리 태그는 의도적으로 숨김: 응답자가 의식적으로 답을 조정하지 못하도록 함 */}
          ...
          <h2 style={{ color: "#F1F5F9", fontSize: 22, fontWeight: 700, lineHeight: 1.5, margin: "0 0 40px" }}>
            {mode === "yesno" && YESNO_TEXT[q?.id] ? YESNO_TEXT[q.id] : q?.text}
          </h2>
...
function GroupedBarChart({ scores }) {
  const GROUPS = [
    {
      label: "대인관계",
      colors: ["#6366F1", "#A855F7", "#38BDF8"], // 인디고 · 퍼플 · 스카이블루
      dims: [
        { key: "E", label: "외향성" },
        { key: "A", label: "친화성" },
        { key: "S", label: "사회성" },
    ...
    {
      label: "업무수행",
      colors: ["#10B981", "#06B6D4", "#84CC16"], // 에메랄드 · 시안 · 라임
      dims: [
        { key: "C", label: "성실성" },
        { key: "P", label: "적극성" },
        { key: "R", label: "리더십" },
    ...
    {
      label: "개인성향",
      colors: ["#F59E0B", "#FB923C", "#FACC15"], // 앰버 · 오렌지 · 옐로우
      dims: [
        { key: "O", label: "개방성" },
        { key: "N", label: "정서\n안정성" },
        { key: "T", label: "스트레스\n내성" },
      ...
      <div style={{ display: "flex", gap: 16 }}>
        {GROUPS.map((group) => (
          <div key={group.label} style={{ flex: 1, minWidth: 0 }}>
            {/* 그룹 레이블 */}
            <div style={{
              textAlign: "center", color: group.colors[0], fontSize: 12, fontWeight: 700,
              marginBottom: 16, padding: "4px 8px",
              background: `${group.colors[0]}15`, borderRadius: 6,
            }}>
              {group.label}
            </div>

            {/* 막대 + 점수 + 라벨을 하나의 컬럼으로 묶어서 동일한 너비로 정렬 */}
            <div style={{ display: "flex", gap: 4 }}>
              {group.dims.map((dim, i) => {
                const score = scores[dim.key] ?? 0;
                const pct = Math.max(0, Math.min(100, (score / maxScore) * 100));
                const barH = Math.max(4, (pct / 100) * chartHeight);
                const barColor = group.colors[i % group.colors.length];
                return (
                  <div key={dim.key} style={{
                    flex: 1, minWidth: 0, display: "flex", flexDirection: "column", alignItems: "center",
                  }}>
                    {/* 점수 */}
                    <div style={{ color: "#F1F5F9", fontSize: 11, fontWeight: 700, marginBottom: 6 }}>
                      {score.toFixed(1)}
                    </div>
                    {/* 막대 영역 (바닥 고정, 높이만 가변) */}
                    <div style={{ width: "100%", height: chartHeight, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
                      <div style={{
                        width: "60%", maxWidth: 32, height: barH, borderRadius: "4px 4px 0 0",
                        background: barColor,
                        transition: "height 0.8s ease",
                      }} />
                    </div>
                    {/* 구분선 */}
                    <div style={{ width: "100%", height: 2, background: `${group.colors[0]}40`, borderRadius: 2, margin: "4px 0 8px" }} />
                    {/* 차원 레이블 */}
                    <div style={{
                      textAlign: "center", color: "#64748B", fontSize: 10, lineHeight: 1.3,
                      whiteSpace: "pre-line",
                    }}>
                      {dim.label}
                    </div>
  ...
  const computeScores = () => {
    ...
    if (mode === "yesno") {
      ...
      for (const q of ALL_QUESTIONS) {
        const raw = answers[q.id]; // 5(예) or 1(아니요)
        if (raw == null) continue;
        const binary = raw === 5 ? 1 : 0; // 예=1, 아니요=0
        const val = q.rev ? 1 - binary : binary; // 역채점
        sums[q.dim] = (sums[q.dim] || 0) + val;
        counts[q.dim] = (counts[q.dim] || 0) + 1;
      }
      const avgs = {};
      for (const dim in sums) {
        // 비율을 1~5 척도로 변환: 0%→1점, 100%→5점
        avgs[dim] = (sums[dim] / counts[dim]) * 4 + 1;
      }
      return avgs;
    } else {
      // 리커트: 기존 방식
